...
|---|---|---|---|---|---|
| TD-S0-002 | 因子验证模块仅完成最小 Gate，完整因子有效性验证仍未实现 | P1 | Validation 统计解释力不足，影响 S3 回测可信度 | S3 | ⏳ 待处理 |
| TD-S0-003 | 权重验证模块仅设计未实现 | P1 | 无法验证组合稳定性 | S3 | ⏳ 待处理 |
...
| TD-S2-009 | IRS/PAS 当前使用最小启发式评分，未接入完整行业映射与收益校准 | P2 | 分数可运行但策略语义偏弱，可能放大回测偏差 | S3 | ⏳ 待处理 |
| TD-S2-010 | S2c 已接入 `validation_weight_plan` 桥接硬门禁，但候选权重生成与 Walk-Forward 选优尚未实现 | P1 | 已能阻断桥接缺失，但仍无法用历史验证结果动态选择最优权重 | S2c | ⏳ 待处理 |
