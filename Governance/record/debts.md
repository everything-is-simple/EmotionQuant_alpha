...
| TD-S3E-020 | `regime + dual-window` 下 `20260126/20260202` 仍触发 `factor_validation_fail`，导致 20 日窗口 `integrated_recommendation` 覆盖仅 `18/20` | P1 | 影响 S3b 扩窗归因完整性与 S4b 参数输入可信度 | S3e | ?? 处理中 |
