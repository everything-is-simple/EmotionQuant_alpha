...
**S2：MSS+IRS+PAS 集成推荐闭环**（S2a 已完成，S2b 待执行）
...
1. 完成 S2a 开发与验证：`eq recommend --date {trade_date} --mode mss_irs_pas --with-validation`，输出 IRS/PAS/Validation 三表最小闭环。
2. 新增 S2a 合同测试：`test_irs_contract.py`、`test_pas_contract.py`、`test_validation_gate_contract.py`；并补 CLI `recommend` 路径回归。
3. 归档 S2a 样例证据到 `Governance/specs/spiral-s2a`。
4. 重跑关键门禁并通过：dependency baseline、S2a 目标测试、contracts/governance、防跑偏回归测试。
...
| S2b | MSS+IRS+PAS 集成推荐闭环 | 🟡 待执行 | 下一圈主目标 |
...
1. 实现 `eq recommend --date {trade_date} --mode integrated` 集成推荐路径。
2. 补齐 `tests/unit/integration/test_integration_contract.py` 与 `tests/unit/integration/test_quality_gate_contract.py`。
3. 产出 `integrated_recommendation_sample.parquet`、`quality_gate_report.md`、`s2_go_nogo_decision.md`。
4. 完成 S2b 的 `requirements/review/final` 与最小同步 5 项。
