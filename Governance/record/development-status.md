...
**最后更新**: 2026-02-15  
**当前版本**: v3.8（S2b 已按 6A 收口，进入 S3 准备）  
...
1. 启动 S3 回测最小闭环需求对齐（命令、测试、产物三件套）。
2. 将 `integrated_recommendation` 作为回测主输入，补齐最小可复现实验。
3. 继续清偿 S2 阶段遗留债务（权重方案接入、行业粒度与评分校准）。
