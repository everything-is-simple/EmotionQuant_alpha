...
**最后更新**: 2026-02-19  
**当前版本**: v4.14（新增 S3ar：采集稳定性修复圈，先收口多源兜底与 DuckDB 锁恢复）  
...
| S3ar | 采集稳定性修复圈（多源兜底 + 锁恢复） | 🔄 进行中 | 修复历史回填阻断项，确保归因窗口数据可复现 |
...
1. 先完成 S3ar：落地多源兜底链路（TuShare -> AKShare -> BaoStock）与 DuckDB 锁恢复门禁。
2. 验证历史窗口可稳定落袋后，启动 S3b：完成 A/B/C 对照与实盘-回测偏差三分解（signal/execution/cost）。
3. 形成收益来源结论并驱动 S4b 参数；继续补齐更细撮合规则（一字板/流动性枯竭）并沉淀可回放证据。
