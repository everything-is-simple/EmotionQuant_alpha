...
> **冻结声明 (2026-02-28)**：本文件为 Spiral S0-S7a 阶段资产快照，仅作为历史参考保留。  
> 新体系下的可复用资产请参考 `docs/roadmap.md`（R0-R9 路线图）与 `docs/cards/`（执行卡）。  
> 本文件不再新增条目。
...
| S-GOV-017 | S3ar 执行卡模板 | `Governance/SpiralRoadmap/execution-cards/S3AR-EXECUTION-CARD.md` | S | 双 TuShare 主备与 DuckDB 锁恢复 run/test/artifact/review/sync 合同复用 |
...
|| S-PAT-065 | 全仓向量化模式库 | PAS: pivot+rolling宽表、Backtest/Trading: set_index+to_dict("index")、Integration/IRS: itertuples+zip向量化、L2: to_dict("records") | S | 消除 iterrows 性能瓶颈的标准模式，可复用于后续新模块 |
...
| 日期 | 版本 | 变更内容 |
|---|---|---|
||| 2026-02-26 | v2.33 | 新增全仓向量化模式库资产（S-PAT-065）：涵盖 PAS pivot+rolling / Backtest+Trading set_index+to_dict / Integration+IRS itertuples / L2 to_dict("records") 四类模式 |
...
| 2026-02-19 | v2.19 | 新增 S3ar 资产登记：`Governance/specs/spiral-s3ar/*` 与 `Governance/SpiralRoadmap/execution-cards/S3AR-EXECUTION-CARD.md`；补充“多源兜底与锁恢复”空缺项 |
