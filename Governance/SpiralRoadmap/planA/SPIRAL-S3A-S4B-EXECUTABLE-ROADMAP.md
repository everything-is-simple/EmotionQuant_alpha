...
本阶段归属于 Plan A 螺旋2（Full 闭环），重点不是“圈位写完”，而是“16年数据+完整回测+完整归因”是否闭环。

### 螺旋2出口必须补齐

1. 历史数据窗口达到 `2008-01-01` 到 `2024-12-31` 并完成质量检查。
2. 完整回测覆盖 1y/3y/5y 及典型市场窗口，可复现可对比。
3. 完整归因交付：A/B/C 对照 + `signal/execution/cost` 三分解。
4. `S3c/S3d/S3e` 与 `S4b` 的参数来源可追溯，禁止经验拍值。
5. `Governance/SpiralRoadmap/planA/PLANA-BUSINESS-SCOREBOARD.md` 的螺旋2项更新为可审计状态。

### 推进限制

1. 螺旋2未通过前，S5 可以开发但不得宣称“可实战/生产就绪”。
2. 若归因结论仍为 `inconclusive`，必须留在 S3b/S3c/S3d/S3e 修复，不得跳到阶段C收口叙事。

---

## 工程实现复核（2026-02-23）

- 复核结论（按“代码完整 + 工程退出门禁”口径）：
...
为避免“有双档定义但无量化标准”，阶段B对 `S3c/S3d/S3e` 增加统一量化门禁，作为螺旋2收口判据：

| 圈位 | MVP（最小可用） | FULL（完整生产口径） |
|---|---|---|
| S3c | `industry_snapshot` 覆盖 31 行业且 IRS 输入字段齐全；允许质量 `WARN`，但需在 `review.md` 给出可执行修复计划 | 31 行业覆盖稳定（近3窗口连续满足），质量告警清零或仅剩非阻断告警且已闭环 |
| S3d | 自适应阈值可运行；样本不足时自动回退固定阈值；`top_bottom_spread_5d` 禁止未来函数且不出现连续2窗负 spread | 自适应阈值在可用样本窗默认启用；关键窗口负 spread 为0；probe 报告可解释且可复跑 |
| S3e | 双窗口 WFA 可运行；`final_gate` 不得 `FAIL`；若 `factor_gate_raw=FAIL` 仅允许在 `neutral_regime` 软化且必须审计说明 | 双窗口 WFA 稳定通过；`factor_gate_raw` 不得 `FAIL`；`selected_weight_plan` 全链可追溯且 OOS 指标达标 |

补充执行约束：

1. `S3c/S3d/S3e` 准备与实验可并行推进。
2. 收口宣告必须串行：`S3c -> S3d -> S3e`。
3. 任一圈未达到 `MVP`，不得推进 `S4b`；未达到 `FULL`，不得宣称“螺旋2生产就绪”。
...
| 版本 | 日期 | 变更说明 |
|---|---|---|
| v1.8 | 2026-02-23 | 新增 `S3c/S3d/S3e` 双档量化门禁（MVP/FULL）并固化“准备并行、收口串行”规则，支撑螺旋2可审计收口 |
| v1.7 | 2026-02-23 | 新增 Reborn 对齐章节：阶段B映射螺旋2（Full），将“16年数据+完整回测+完整归因”设为出口硬门禁；状态从 Completed 调整为 Active（执行中） |
