...
def build_parser() -> argparse.ArgumentParser:
    ...
    fetch_batch_parser.add_argument(
        ...
        default="day",
        help="Batch unit for --batch-size: day/month/year (default day).",
    )
    fetch_batch_parser.add_argument(
        "--workers",
        type=int,
    ...
    gui_parser.add_argument(
        "--no-launch",
        action="store_true",
        help="Validate GUI entry without starting Streamlit server.",
    )
    gui_parser.add_argument(
        "--port",
        type=int,
        default=None,
        help="Override Streamlit server port for this run.",
    )
    gui_parser.add_argument(
        "--export",
        ...
        json.dumps(
            {
                ...
                "current_end_date": result.current_end_date,
                "current_trade_date": result.current_trade_date,
                "current_trade_index": result.current_trade_index,
                "current_trade_total": result.current_trade_total,
                "artifacts_dir": str(result.artifacts_dir),
                "progress_path": str(result.progress_path),
            },
            ensure_ascii=True,
...
def _run_gui(ctx: PipelineContext, args: argparse.Namespace) -> int:
    try:
        result = run_gui(
            ...
            launch_dashboard=not bool(args.no_launch),
    ...
    dashboard_url = getattr(result, "dashboard_url", None)
    if dashboard_url:
        payload["dashboard_url"] = str(dashboard_url)
    print(json.dumps(payload, ensure_ascii=True, sort_keys=True))
    return 1 if result.has_error else 0
