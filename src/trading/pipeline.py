...
def run_paper_trade(
    *,
    ...
    available_cash = float(config.backtest_initial_cash)
    ...
    def try_execute_buy(row: dict[str, object], idx: int) -> tuple[bool, bool]:
        ...
        if _is_limit_up(price):
            ...
            trade_records.append(
                {
                    "trade_id": f"TRD_{trade_date}_{idx:04d}",
                    "trade_date": trade_date,
                    "stock_code": stock_code,
        ...
        current_total_position = sum(float(item["market_value"]) for item in positions)
        if (current_total_position / max(1.0, available_cash + current_total_position)) > max_total_position:
            risk_events.append(
                {
                    "trade_date": trade_date,
                    "stock_code": stock_code,
        ...
        positions.append(
            {
                ...
                "market_price": round(float(price.get("close", filled_price)), 4),
                "market_value": round(float(price.get("close", filled_price)) * shares, 4),
                "buy_date": trade_date,
    ...
    if not errors:
        ...
        any_filled = False
        for idx, payload in enumerate(strict_candidates, start=1):
            filled, _ = try_execute_buy(payload, idx)
        ...
        if not any_filled and fallback_candidates:
            ...
            try_execute_buy(fallback_candidates[0], 9999)
    ...
    filled_orders = (
        int((trade_frame["status"] == "filled").sum()) if not trade_frame.empty else 0
    )
    total_orders = int(len(trade_frame))
    risk_event_count = int(len(risk_event_frame))

    quality_status = "FAIL" if errors else ("WARN" if risk_event_count > 0 else "PASS")
    go_nogo = "NO_GO" if quality_status == "FAIL" else "GO"
    if not errors and filled_orders <= 0:
        quality_status = "FAIL"
        go_nogo = "NO_GO"
